...
async def tienda(ctx):
    ...
    async def select_callback(interaction):
        ...
        modo = interaction.data['values'][0]
        view = View()
            ...
            async def item_callback(i, nombre=nombre):
                ...
                if modo == "comprar":
                    if inv["coronas"] >= precio:
                        ...
                        await log_event(TIENDA_LOG_CHANNEL_ID, f"🛒 **{ctx.author.display_name}** compró **{nombre}** por {precio} 🪙")
                    ...
                    if inv.get(nombre, 0) > 0:
                        ...
                        await log_event(TIENDA_LOG_CHANNEL_ID, f"💰 **{ctx.author.display_name}** vendió **{nombre}** por {precio // 2} 🪙")
                    else:
        ...
        await interaction.response.send_message(f"Modo: **{modo}** — elige un objeto:", view=view, ephemeral=True)
